...
:cljs
   (do
     ...
     (def subdomain #"(?i)^[a-z\xa0-\ud7ff\uf900-\ufdcf\ufdf0-\uffef](?:[a-z0-9-\xa0-\ud7ff\uf900-\ufdcf\ufdf0-\uffef]{0,61}[a-z0-9\xa0-\ud7ff\uf900-\ufdcf\ufdf0-\uffef])?(?:\.[a-z0-9\xa0-\ud7ff\uf900-\ufdcf\ufdf0-\uffef](?:[a-z0-9-\xa0-\ud7ff\uf900-\ufdcf\ufdf0-\uffef]{0,61}[a-z0-9\xa0-\ud7ff\uf900-\ufdcf\ufdf0-\uffef])?)*$")
   ...
   ;; label lengths) is limited to 255." -- RFC 1034, Section 3.1
     ...
     (defn idn-hostname? [s]
       (when (empty? (set/intersection (set (seq s))
                                       #{\u302E ; Hangul single dot tone mark
                                         }))
     ;; Ensure ASCII version is a valid hostname
         (hostname? s)))
